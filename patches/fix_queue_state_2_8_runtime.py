...
grep -q "pipeline_depth.*4" "$ROOT/admin.php"
grep -Fq "version_compare($schemaVersion, '2.8.0-queue-state', '<')" "$ROOT/app.php"
grep -Fq "App::db();' \"$INSTALL_DIR/app.php\"" "$ROOT/update.sh" || grep -Fq "App::db();' \"$ROOT/app.php\"" "$ROOT/update.sh" || true
