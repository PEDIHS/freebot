...
for cmd in php nginx mariadb aria2c ffmpeg ffprobe mediainfo yt-dlp; do check_cmd "$cmd"; done
...
if [[ -x /opt/freebot-tools/bin/python ]] && /opt/freebot-tools/bin/python -c 'import telethon, hachoir' >/dev/null 2>&1; then pass "Telethon video metadata support"; else fail "Telethon or hachoir missing"; fi
if [[ -f "$INSTALL_DIR/config.php" ]]; then
  # shellcheck disable=SC2016
  if php -r '$c=require $argv[1];$d=$c["db"];$p=new PDO("mysql:host={$d["host"]};dbname={$d["name"]};charset=utf8mb4",$d["user"],$d["pass"]);foreach(["media_jobs","media_workers","media_job_events","channel_stats"] as $t){$p->query("SELECT 1 FROM `$t` LIMIT 1");}foreach(["file_path","error_message","source_chat_id","source_message_id","source_date","target_channel_id","target_slot","target_sequence"] as $n){if(!$p->query("SHOW COLUMNS FROM media_jobs LIKE ".$p->quote($n))->fetch())throw new RuntimeException($n);}foreach(["source_type","source_channel_id","source_channel_title","source_last_message_id","source_scanned_items","source_skipped_items","sequential_mode","pipeline_depth","distribution_mode","destination_channels_json","destination_limit","overflow_items","scan_status","scan_attempts","scan_max_attempts","scan_next_attempt_at","scan_error","scan_locked_by","scan_lock_token","scan_lock_expires_at","scan_heartbeat_at","scan_options_json"] as $n){if(!$p->query("SHOW COLUMNS FROM media_batches LIKE ".$p->quote($n))->fetch())throw new RuntimeException($n);}foreach(["history_last_message_id","history_message_count","history_video_count","history_photo_count","history_file_count","history_total_bytes","history_scan_status","history_scan_error","history_scanned_at"] as $n){if(!$p->query("SHOW COLUMNS FROM channel_stats LIKE ".$p->quote($n))->fetch())throw new RuntimeException($n);}$u=$p->query("SHOW COLUMNS FROM media_jobs LIKE ".$p->quote("url"))->fetch();if($u&&($u["Null"]??"NO")!=="YES")throw new RuntimeException("legacy url is still required");' "$INSTALL_DIR/config.php"; then pass "Database and Media schema"; else fail "Database or Media schema"; fi
