...
cat "$REL"/freebot.tar.gz.b64.part-* > "$TMP/base.b64"
...
cat "$REL"/media-overlay.patch.gz.b64.part-* > "$TMP/overlay.b64"
...
( cd "$TMP/src" && patch -p1 --batch --forward < "$TMP/overlay.patch" ) || fail "اعمال overlay ناموفق بود."
...
php "$TMP/src/tests/run.php" >/tmp/freebot-update-tests.log 2>&1 || { tail -80 /tmp/freebot-update-tests.log >&2; fail "تست پروژه ناموفق بود."; }
