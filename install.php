...
CREATE TABLE IF NOT EXISTS media_batches (id bigint unsigned AUTO_INCREMENT PRIMARY KEY,product_id int unsigned NULL,channel_id varchar(64) NOT NULL,title varchar(255) NOT NULL,caption_template text NULL,upload_mode enum('auto','video','document') NOT NULL DEFAULT 'auto',source_type varchar(30) NOT NULL DEFAULT 'links',source_channel_id varchar(64) NULL,source_channel_title varchar(255) NULL,source_last_message_id bigint unsigned NOT NULL DEFAULT 0,source_scanned_items bigint unsigned NOT NULL DEFAULT 0,source_video_count bigint unsigned NOT NULL DEFAULT 0,sequential_mode tinyint(1) NOT NULL DEFAULT 0,pipeline_depth tinyint unsigned NOT NULL DEFAULT 1,distribution_mode varchar(20) NOT NULL DEFAULT 'single',destination_channels_json longtext NULL,destination_limit int unsigned NOT NULL DEFAULT 0,overflow_items int unsigned NOT NULL DEFAULT 0,scan_status varchar(20) NOT NULL DEFAULT 'completed',scan_attempts tinyint unsigned NOT NULL DEFAULT 0,scan_max_attempts tinyint unsigned NOT NULL DEFAULT 3,scan_next_attempt_at datetime NULL,scan_error text NULL,scan_locked_by varchar(190) NULL,scan_lock_token char(64) NULL,scan_lock_expires_at datetime NULL,scan_heartbeat_at datetime NULL,scan_options_json longtext NULL,status enum('queued','running','paused','completed','completed_with_errors','cancelled') NOT NULL DEFAULT 'queued',total_items int unsigned NOT NULL DEFAULT 0,completed_items int unsigned NOT NULL DEFAULT 0,failed_items int unsigned NOT NULL DEFAULT 0,current_item_id bigint unsigned NULL,created_by varchar(64) NOT NULL DEFAULT 'panel',started_at datetime NULL,completed_at datetime NULL,notification_status varchar(30) NULL,notification_sent_at datetime NULL,created_at datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,INDEX(status),INDEX idx_media_batch_scan(scan_status,scan_next_attempt_at,scan_lock_expires_at),INDEX(product_id),CONSTRAINT fk_media_batch_product FOREIGN KEY(product_id) REFERENCES products(id) ON DELETE SET NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
CREATE TABLE IF NOT EXISTS media_jobs (id bigint unsigned AUTO_INCREMENT PRIMARY KEY,batch_id bigint unsigned NOT NULL,position int unsigned NOT NULL,source_url text NOT NULL,source_host varchar(255) NOT NULL DEFAULT '',detected_title varchar(500) NULL,engine varchar(50) NULL,status enum('queued','downloading','downloaded','uploading','completed','failed','cancelled') NOT NULL DEFAULT 'queued',progress decimal(5,2) NOT NULL DEFAULT 0,attempts tinyint unsigned NOT NULL DEFAULT 0,download_attempts tinyint unsigned NOT NULL DEFAULT 0,upload_attempts tinyint unsigned NOT NULL DEFAULT 0,max_attempts tinyint unsigned NOT NULL DEFAULT 3,next_attempt_at datetime NULL,downloaded_bytes bigint unsigned NOT NULL DEFAULT 0,total_bytes bigint unsigned NOT NULL DEFAULT 0,download_speed_bps bigint unsigned NOT NULL DEFAULT 0,upload_speed_bps bigint unsigned NOT NULL DEFAULT 0,eta_seconds int unsigned NULL,file_path varchar(1000) NULL,file_name varchar(500) NULL,mime_type varchar(120) NULL,source_chat_id varchar(64) NULL,source_message_id bigint NULL,source_date datetime NULL,target_channel_id varchar(64) NULL,target_slot tinyint unsigned NULL,target_sequence int unsigned NULL,telegram_message_id bigint NULL,error_code varchar(80) NULL,error_message text NULL,locked_by varchar(190) NULL,lock_token char(64) NULL,lock_expires_at datetime NULL,heartbeat_at datetime NULL,started_at datetime NULL,finished_at datetime NULL,created_at datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY uniq_media_batch_position(batch_id,position),INDEX idx_media_job_pick(status,next_attempt_at,lock_expires_at,id),INDEX idx_media_tg_source(source_chat_id,source_message_id),INDEX idx_media_target(target_channel_id,status),CONSTRAINT fk_media_job_batch FOREIGN KEY(batch_id) REFERENCES media_batches(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS channel_stats (product_id int unsigned PRIMARY KEY,channel_id varchar(64) NOT NULL,channel_title varchar(255) NULL,channel_username varchar(255) NULL,member_count int unsigned NOT NULL DEFAULT 0,admin_count int unsigned NOT NULL DEFAULT 0,bot_status varchar(30) NULL,can_post tinyint(1) NOT NULL DEFAULT 0,last_error text NULL,refreshed_at datetime NULL,history_last_message_id bigint unsigned NOT NULL DEFAULT 0,history_message_count bigint unsigned NOT NULL DEFAULT 0,history_video_count bigint unsigned NOT NULL DEFAULT 0,history_photo_count bigint unsigned NOT NULL DEFAULT 0,history_file_count bigint unsigned NOT NULL DEFAULT 0,history_total_bytes bigint unsigned NOT NULL DEFAULT 0,history_scan_status varchar(30) NOT NULL DEFAULT 'never',history_scan_error text NULL,history_scanned_at datetime NULL,CONSTRAINT fk_channel_stats_product FOREIGN KEY(product_id) REFERENCES products(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
SQL;
        ...
        $settings=[
            ...
            'downloader_batch_limit'=>'100','media_download_timeout'=>'21600','media_upload_timeout'=>'21600','media_lock_seconds'=>'180','media_scan_lease_seconds'=>'600',
            'media_download_connections'=>'8','media_fragment_concurrency'=>'8','channel_history_scan_timeout'=>'7200','telegram_mtproto_upload'=>'1','schema_version'=>'2.5.2-telegram-inventory'
        ];
        ...
        // Repair a pre-existing 1.x Media schema before workers receive jobs.
        App::db();
