...
نسخه `2.4.1-async-channel-scan` بر پایه مستقیم پروژه PHP فروش فیلم `1.3.1-product-grid` ساخته شده است. این ریپو هیچ وابستگی یا فایلی از پروژه‌های VPN، Proxy، Overlay یا Hotfixهای قدیمی ندارد.
...
- انتقال پایدار ویدیوهای کانال مبدأ با اسکن قدیمی به جدید و ثبت دائمی مقصد هر Job
- تقسیم بلوکی ویدیوها بین ۲ یا ۳ کانال مقصد با ظرفیت قابل‌تنظیم هر کانال و آمار مجزا
- Pipeline محدود ۱ تا ۴ فایل برای همپوشانی دانلود/آپلود بدون پر شدن دیسک
- جلوگیری اختیاری از انتقال دوباره ویدیوهایی که قبلاً برای همان محصول وارد شده‌اند
- اسکن انتقالی کاملاً مستقل از مرورگر با Worker، Heartbeat، Lease، Retry و بازیابی خودکار پس از Restart
- مهاجرت خودکار جدول‌های قدیمی دارای ستون اجباری `url` و مصونیت صف در برابر collationهای متفاوت
...
## انتقال کامل ویدیوهای یک کانال

بعد از اتصال حساب Telethon، در «دانلودر و آپلودر» کارت «انتقال کامل ویدیوهای یک کانال تلگرام» فعال می‌شود. آیدی عددی کانال مبدأ (مانند `-1001234567890`) و محصول مقصد را انتخاب کنید. حساب متصل باید به مبدأ دسترسی داشته باشد و ربات باید در کانال مقصد ادمین و دارای اجازه ارسال باشد.

سیستم ابتدا درخواست را در دیتابیس ثبت می‌کند و بلافاصله کنترل را به پنل برمی‌گرداند. پس از آن Download Worker تاریخچه را از قدیمی‌ترین پیام تا جدیدترین پیام می‌خواند؛ بستن تب، خروج از پنل یا قطع اینترنت مرورگر اسکن را متوقف نمی‌کند. Worker فقط ویدیوها را ثبت می‌کند و برای هر یک آدرس داخلی `tgmtproto` و کانال مقصد ثابت در دیتابیس می‌نویسد. Lease و Heartbeat اسکن مانع اجرای تکراری می‌شوند و پس از Restart، اسکن ناقص با Retry و پاک‌سازی Jobهای نیمه‌ساخته از ابتدا بازسازی می‌شود. با فعال‌کردن تقسیم، مقصد اول همان کانال محصول است و مقصد دوم/سوم با آیدی عددی اضافه می‌شوند. با ظرفیت ۲۰۰۰، بلوک اول ۲۰۰۰ ویدیو به مقصد اول و بلوک بعدی به مقصد دوم می‌رود. Pipeline قابل‌تنظیم اجازه می‌دهد دانلود بعدی همزمان با آپلود قبلی انجام شود، اما تعداد فایل‌های روی دیسک محدود می‌ماند. آپلود بدون Caption است و restart، Heartbeat و Retry مقصد یک Job را عوض نمی‌کنند.

اگر حساب تلگرام رمز دومرحله‌ای فعال نداشته باشد، مرحله رمز نمایش داده نمی‌شود و اتصال بلافاصله بعد از کد کامل می‌شود.
...
- تست یکپارچه MariaDB برای Queue، Lock، Heartbeat، Retry، ترتیب انتقال کانال و وضعیت‌های Job
