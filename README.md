...
نسخه `2.4.0-channel-distribution` بر پایه مستقیم پروژه PHP فروش فیلم `1.3.1-product-grid` ساخته شده است. این ریپو هیچ وابستگی یا فایلی از پروژه‌های VPN، Proxy، Overlay یا Hotfixهای قدیمی ندارد.
...
سیستم تاریخچه را از قدیمی‌ترین پیام تا جدیدترین پیام می‌خواند، فقط ویدیوها را ثبت می‌کند و برای هر یک آدرس داخلی `tgmtproto` و کانال مقصد ثابت در دیتابیس می‌نویسد. با فعال‌کردن تقسیم، مقصد اول همان کانال محصول است و مقصد دوم/سوم با آیدی عددی اضافه می‌شوند. با ظرفیت ۲۰۰۰، بلوک اول ۲۰۰۰ ویدیو به مقصد اول و بلوک بعدی به مقصد دوم می‌رود. اگر تعداد از ظرفیت کل بیشتر باشد، ساخت صف کاملاً رد می‌شود. Pipeline قابل‌تنظیم اجازه می‌دهد دانلود بعدی همزمان با آپلود قبلی انجام شود، اما تعداد فایل‌های روی دیسک محدود می‌ماند. آپلود بدون Caption است و restart، Heartbeat و Retry مقصد یک Job را عوض نمی‌کنند.
