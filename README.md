...
نسخه `2.3.6-telethon-ascii-json` بر پایه مستقیم پروژه PHP فروش فیلم `1.3.1-product-grid` ساخته شده است. این ریپو هیچ وابستگی یا فایلی از پروژه‌های VPN، Proxy، Overlay یا Hotfixهای قدیمی ندارد.
...
- انتقال کامل ویدیوهای کانال مبدأ با آیدی عددی: اسکن قدیمی به جدید، منبع داخلی امن برای هر پیام و صف دانلود/آپلود کاملاً ترتیبی
...
سیستم تاریخچه را از قدیمی‌ترین پیام تا جدیدترین پیام می‌خواند، برای هر ویدیو یک آدرس داخلی `tgmtproto` ثبت می‌کند و آن‌ها را در صف پایدار قرار می‌دهد. در این نوع دسته، ویدیوی بعدی فقط بعد از تکمیل دانلود و آپلود ویدیوی قبلی شروع می‌شود. آپلود ربات بدون Caption است و پس از Restart نیز صف ادامه پیدا می‌کند. آدرس داخلی لینک عمومی یا قابل اشتراک نیست و فقط Worker دارای نشست مجاز می‌تواند محتوا را دریافت کند.
