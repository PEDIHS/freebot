...
نسخه `2.5.1-resumable-checkpoints` بر پایه مستقیم پروژه PHP فروش فیلم `1.3.1-product-grid` ساخته شده است. این ریپو هیچ وابستگی یا فایلی از پروژه‌های VPN، Proxy، Overlay یا Hotfixهای قدیمی ندارد.
...
- آپلود سریع MTProto بدون Caption تا سقف سخت ۱ گیگابایت، همراه Retry و پیشرفت لحظه‌ای
...
- انتقال سخت‌گیرانه تک‌فایل: دانلود یک ویدیو، آپلود فوری، حذف از دیسک و سپس شروع ویدیوی بعدی
- Checkpoint زنده اسکن بر اساس آخرین `message_id`؛ ادامه پس از خطا، توقف یا Restart بدون ساخت دوباره Jobهای قبلی
...
برای انتقال کانال، حساب Telethon متصل باید عضو کانال مبدأ و دارای اجازه ارسال در تمام کانال‌های مقصد باشد. نصب و آپدیت، `cryptg` را نیز برای کاهش سربار رمزنگاری Telethon نصب می‌کند.
