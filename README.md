...
نسخه `2.1.0-channel-history` بر پایه مستقیم پروژه PHP فروش فیلم `1.3.1-product-grid` ساخته شده است. این ریپو هیچ وابستگی یا فایلی از پروژه‌های VPN، Proxy، Overlay یا Hotfixهای قدیمی ندارد.
...
Bot API تاریخچه قبل از اضافه‌شدن ربات را ارائه نمی‌کند. برای فعال‌کردن اسکن کامل، بعد از آپدیت این دستور را یک‌بار روی سرور اجرا کنید:

```bash
sudo /var/www/freebot/setup-channel-scanner.sh
```

`API ID` و `API Hash` را از `my.telegram.org` بگیرید و شماره حساب تلگرامی را وارد کنید که عضو کانال‌های محصولات است. کد ورود و در صورت فعال‌بودن، رمز دومرحله‌ای تلگرام پرسیده می‌شود. نشست خارج از مسیر وب و با دسترسی محدود ذخیره می‌شود.
