...
$payload=null;for($index=count($lines)-1;$index>=0;$index--){$decoded=json_decode(trim($lines[$index]),true,512,JSON_BIGINT_AS_STRING|JSON_INVALID_UTF8_SUBSTITUTE);if(is_array($decoded)){$payload=$decoded;break;}}
if(!is_array($payload)||!($payload['ok']??false)||!hash_equals($token,(string)($payload['probe']??''))||($payload['unicode']??'')!=='تست 🎬')throw new RuntimeException('Transport probe returned invalid JSON/UTF-8: '.implode("\n",$lines));
